...
const HomeNavbar = () => {
  ...
  const navLink = (to, label, isActive) => (
    <Link
      ...
      className={`relative pb-1 text-sm font-medium text-white/95 transition hover:text-white ${isActive ? "after:absolute after:bottom-0 after:left-0 after:h-0.5 after:w-full after:bg-[#FF7A00]" : ""
        }`}
    >
      {label}
    </Link>
  ...
  return (
    <div className="fixed bg-black w-full  opacity-100 md:opacity-60 z-50 p-3 ">
      <div className="flex max-w-w-full items-center justify-between gap-4">
        ...
        <div className="hidden items-center gap-3 md:flex">
          ...
          {user ? (
            ...
            onClick={clickHandler}
              className="rounded-lg  text-black bg-[#ff0000] px-5 py-2.5 text-sm font-semibold  shadow-sm transition hover:bg-[#E66D00]"
            >
              Logout
            </button>
          ) : (
